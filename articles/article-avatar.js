...
link.rel='stylesheet';
link.href='article-avatar.css?v=20260915-5';
document.head.appendChild(link);

const box=document.createElement('div');
box.id='paperAvatarWidget';
box.innerHTML='<div class="avatar-bubble"><div class="avatar-chip">Explicação guiada</div><div class="avatar-title">Resumo do artigo</div><p class="avatar-text">Olá! Eu sou a Rayana virtual. Ative a voz e eu vou explicando o artigo à medida que você desce a página.</p><div class="avatar-status">Áudio desligado. O texto já acompanha a rolagem.</div><div class="voice-row"><label for="rayanaVoiceSelect">Voz feminina</label><select id="rayanaVoiceSelect" class="voice-select"><option value="">Carregando vozes...</option></select></div><div class="avatar-controls"><button class="primary voice" type="button">Ativar voz</button><button class="test" type="button">Testar voz</button><button class="replay" type="button">Repetir</button><button class="min" type="button">Minimizar</button></div></div><div class="avatar-wrap"><div class="avatar-glow"></div><img class="avatar-img" src="avatar-rayana.svg?v=20260915-5" alt="Avatar ilustrado de Rayana Schneider"></div>';
...
let voiceOn=false,current=items[0],last='',selectedVoice=null,availableVoices=[];
...
function femaleScore(v){
 const name=normalize((v.name||'')+' '+(v.voiceURI||''));const lang=normalize(v.lang||'');
 ...
 let score=0;if(lang.startsWith('pt-br'))score+=200;else if(lang.startsWith('pt'))score+=80;
...
function refreshVoices(){
 ...
 voiceSelect.innerHTML='<option value="">Selecione uma voz feminina...</option>';
 availableVoices.forEach((v,i)=>{const o=document.createElement('option');o.value=String(i);o.textContent=(v.name||'Voz')+(v.lang?' · '+v.lang:'');voiceSelect.appendChild(o)});
 const saved=localStorage.getItem('rayanaAvatarVoiceName');let idx=-1;
 ...
 else{voiceSelect.value='';selectedVoice=null;}
 if(voiceOn){status.textContent=selectedVoice?'Voz selecionada: '+selectedVoice.name+' · '+selectedVoice.lang+'.':'Escolha uma voz feminina no menu antes de ativar o áudio.';}
}

voiceSelect.addEventListener('change',()=>{const raw=voiceSelect.value;if(raw===''){selectedVoice=null;stop();status.textContent='Escolha uma voz feminina no menu.';return}const i=Number(raw);selectedVoice=availableVoices[i]||null;if(selectedVoice){localStorage.setItem('rayanaAvatarVoiceName',selectedVoice.name);status.textContent='Voz selecionada: '+selectedVoice.name+' · '+selectedVoice.lang+'.'}stop();});
function stop(){if('speechSynthesis'in window)speechSynthesis.cancel();box.classList.remove('speaking')}
function speak(t){if(!voiceOn||!('speechSynthesis'in window)||!selectedVoice)return;stop();const u=new SpeechSynthesisUtterance(t);u.voice=selectedVoice;u.lang=selectedVoice.lang||'pt-BR';u.rate=.96;u.pitch=1.05;u.volume=1;u.onstart=()=>{box.classList.add('speaking');status.textContent='Falando com: '+selectedVoice.name+' · '+selectedVoice.lang+'.'};u.onend=()=>box.classList.remove('speaking');u.onerror=()=>box.classList.remove('speaking');speechSynthesis.speak(u)}

voiceBtn.addEventListener('click',()=>{if(!voiceOn){refreshVoices();if(!selectedVoice){status.textContent='Selecione uma voz feminina no menu. Não vou usar uma voz masculina como alternativa.';return}voiceOn=true;voiceBtn.textContent='Desativar voz';last='';setItem(active(),true)}else{voiceOn=false;voiceBtn.textContent='Ativar voz';status.textContent='Áudio desligado. O texto continua acompanhando a rolagem.';stop()}});
testBtn.addEventListener('click',()=>{refreshVoices();if(!selectedVoice){status.textContent='Selecione uma voz feminina no menu para testar.';return}const old=voiceOn;voiceOn=true;speak('Olá, eu sou a Rayana virtual. Esta é a voz selecionada para explicar o artigo.');voiceOn=old});
...
if(!('speechSynthesis'in window)){voiceBtn.disabled=true;testBtn.disabled=true;replayBtn.disabled=true;voiceSelect.disabled=true;status.textContent='Seu navegador não oferece síntese de voz; o resumo em texto continua funcionando.';}else{refreshVoices();speechSynthesis.onvoiceschanged=refreshVoices;setTimeout(refreshVoices,300);setTimeout(refreshVoices,1200)}
let ticking=false;addEventListener('scroll',()=>{if(!ticking){requestAnimationFrame(()=>{const a=active();if(a.title!==current.title)setItem(a,voiceOn);ticking=false});ticking=true}},{passive:true});
addEventListener('beforeunload',stop);setItem(active(),false);
})();